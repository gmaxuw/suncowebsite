...
export interface PaymentRecord {
  year: number; date: string; amount: number; receipt: string; type: string;
}
export interface MemberRecord {
  no: number; name: string; status: string; member_id_code: string;
  date_joined: string; payments: PaymentRecord[];
  years_delinquent: number; total_amount: number;
}
export interface FeeSchedule {
  year: number; fee_mas: number; fee_aof: number; fee_lifetime: number;
}
export interface OfficerRecord {
  name: string; role: string; role_type: string;
}
...
// ── Default fallback (used if settings not passed) ────────────
...
// ── Helpers ───────────────────────────────────────────────────
function buildOrgLines(org: OrgSettings) {
  return {
    ORG_NAME:   `${org.org_name} (${org.org_short_name})`,
    ORG_SEC:    `SEC ${org.org_sec_number}`,
    ORG_DTI:    `Accredited Partner — Department of Trade and Industry (DTI), ${org.org_region}`,
    ORG_EMAIL:  org.org_email,
    ORG_MOBILE: org.org_phone,
    ORG_ADDR:   org.org_address,
  };
}
...
function paymentsByYearType(payments: PaymentRecord[], year: number, type: string): PaymentRecord[] {
...
function peso(amount: number) {
  return amount > 0 ? `₱${amount.toLocaleString("en-PH", { minimumFractionDigits: 2 })}` : "—";
}
function pesoPDF(amount: number) {
  return amount > 0 ? `PHP ${amount.toLocaleString("en-PH", { minimumFractionDigits: 2 })}` : "—";
}
function getRate(feeSchedules: FeeSchedule[], year: number, type: "mas" | "aof" | "lifetime"): number {
...
function orgHeader(org: OrgSettings, label: string): any[][] {
  const { ORG_NAME, ORG_SEC, ORG_DTI, ORG_EMAIL, ORG_MOBILE } = buildOrgLines(org);
  return [
    [ORG_NAME],
    [ORG_SEC + "   |   " + ORG_DTI],
...
export function exportToExcel(
  ...
  const org   = { ...DEFAULT_ORG, ...orgSettings };
  const wb    = XLSX.utils.book_new();
  const years = getAllYears(members);
  const type  = activeTab;

  const sheetNames: Record<ActiveTab, string> = {
    all: "All Payments", mas: "MAS Only", aof: "Operating Fund", lifetime: "Lifetime",
  };
  ...
  const rows: any[][] = [...orgHeader(org, TAB_LABELS[type])];
  const ORG_ROW_COUNT = 6;

  const header: any[] = ["NO.", "NAME", "STATUS", "MEMBER ID", "DATE JOINED"];
  if (type === "all") {
    years.forEach((yr) => header.push(`${yr} MAS DATE`, `${yr} MAS AMT`, `${yr} AOF DATE`, `${yr} AOF AMT`));
  } else {
  ...
  for (let r = 0; r < 5; r++) {
    for (let c = 0; c < totalCols; c++) {
      setStyle(r, c, {
        ...
        font: { color: { argb: r === 3 ? C_GOLD : "FFC8C8C8" }, bold: r === 0 || r === 3, size: r === 0 ? 12 : 8 },
        border: thinBorder,
  ...
  const firstYrCol = 5;
  const lastYrCol  = type === "all" ? firstYrCol + years.length * 4 : firstYrCol + years.length * 2;
  ...
  members.forEach((m, mi) => {
    ...
    for (let c = 0; c < totalCols; c++) {
      ...
      if (c >= firstYrCol && c < lastYrCol) {
        if (raw === "N/A") { cellFill = fill(C_GREY_NA); font = { ...font, italic: true, color: { argb: "FF8C8C8C" } }; align = { ...align, horizontal: "center" }; }
        else if (raw === "" || raw === "0") { cellFill = fill(C_YELLOW); font = { ...font, color: { argb: "FFA06400" } }; }
      } else if (c === delinqCol) {
  ...
  for (let c = 0; c < totalCols; c++) {
    setStyle(totIdx, c, { fill: fill(C_GREEN_TOT), font: { bold: true, color: { argb: C_GREEN_DK }, size: 8 }, alignment: { horizontal: c > 1 ? "right" : "left", vertical: "middle" }, border: thinBorder });
  }
  const dCIdx = totIdx + 2;
  for (let c = 0; c < totalCols; c++) {
    setStyle(dCIdx, c, { fill: fill(C_RED_LT), font: { bold: true, color: { argb: "FFA01414" }, size: 8 }, alignment: { horizontal: c > 1 ? "center" : "left", vertical: "middle" }, border: thinBorder });
  }
  const dAIdx = dCIdx + 1;
  for (let c = 0; c < totalCols; c++) {
    setStyle(dAIdx, c, { fill: fill("FFFFD7D7"), font: { bold: true, color: { argb: C_RED_DK }, size: 8 }, alignment: { horizontal: c > 1 ? "right" : "left", vertical: "middle" }, border: thinBorder });
  }
  const sigNameIdx  = dAIdx + 4;
  const sigTitleIdx = sigNameIdx + 1;
  [0, 2, 4].forEach((c) => {
    setStyle(sigNameIdx,  c, { font: { bold: true, underline: true, color: { argb: C_GREEN_DK }, size: 9 }, alignment: { horizontal: "center" } });
    setStyle(sigTitleIdx, c, { font: { italic: true, color: { argb: "FF666666" }, size: 8 }, alignment: { horizontal: "center" } });
  });

  if (!ws["!merges"]) ws["!merges"] = [];
  for (let r = 0; r < 5; r++) ws["!merges"].push({ s: { r, c: 0 }, e: { r, c: totalCols - 1 } });
  ws["!merges"].push({ s: { r: dAIdx + 2, c: 0 }, e: { r: dAIdx + 2, c: totalCols - 1 } });
...
export function exportToCSV(
  ...
  const org   = { ...DEFAULT_ORG, ...orgSettings };
  const { ORG_NAME, ORG_SEC, ORG_EMAIL, ORG_MOBILE } = buildOrgLines(org);
  const years = getAllYears(members);
  const type  = activeTab;
  const rows: any[][] = [];
...
export async function exportToPDF(
  ...
  const LOGO_X = 8, LOGO_Y = 4, LOGO_SIZE = 30;
  ...
  if (logoBase64) {
    ...
    doc.text(org.org_short_name.charAt(0), LOGO_X + 7.5, LOGO_Y + 16);
  }
  ...
  if (type === "all") {
    ...
    body = members.map((m) => {
      const jy = joinYear(m);
      const row: any[] = [m.no, m.name, m.status, m.date_joined ? String(m.date_joined).slice(0, 7) : "—"];
      years.forEach((yr) => {
        if (jy > 0 && yr < jy) { row.push("N/A", "N/A"); return; }
        const mas = paymentsByYearType(m.payments, yr, "mas")[0];
        const aof = paymentsByYearType(m.payments, yr, "aof")[0];
    ...
    head = [["No.", "Name", "Status", "Joined", ...years.map((yr) => `${yr}`), "Delinquent", "Total"]];
  ...
  autoTable(doc, {
    ...
    didParseCell: (data) => {
      ...
      if (isTotal)   { data.cell.styles.fontStyle = "bold"; data.cell.styles.fillColor = [220,235,220]; data.cell.styles.textColor = GREEN_DK; }
      if (isDelinqC) { data.cell.styles.fontStyle = "bold"; data.cell.styles.fillColor = RED_LIGHT; data.cell.styles.textColor = [160,20,20]; }
      if (isDelinqA) { data.cell.styles.fontStyle = "bold"; data.cell.styles.fillColor = [255,215,215]; data.cell.styles.textColor = [140,0,0]; }
      if (data.section === "body" && data.row.index < body.length && data.column.index >= 4 && data.column.index < lastDataCol && String(data.cell.raw) === "N/A") {
        data.cell.styles.fillColor = [220,220,220]; data.cell.styles.textColor = [140,140,140]; data.cell.styles.fontStyle = "italic";
      }
      if (data.section === "body" && data.row.index < body.length && data.column.index >= 4 && data.column.index < lastDataCol && String(data.cell.raw) === "—") {
        data.cell.styles.fillColor = YELLOW; data.cell.styles.textColor = [160,100,0];
      }
  ...
  sigCols.forEach(({ name, title }, i) => {
    const x  = 14 + i * colW;
    const cx = x + colW / 2;
    doc.setDrawColor(...GREEN_DK); doc.setLineWidth(0.3);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      `Page ${i} of ${pageCount}  —  ${org.org_short_name} CONFIDENTIAL  —  SEC ${org.org_sec_number}`,
      14, PAGE_W - 4
    );
  }
