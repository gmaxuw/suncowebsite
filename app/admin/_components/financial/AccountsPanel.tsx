"use client";
// ─────────────────────────────────────────────────────────────
// AccountsPanel.tsx — CRUD for bank accounts & cash on hand
// Place at: D:\suncowebsite\app\admin\_components\financial\AccountsPanel.tsx
// ─────────────────────────────────────────────────────────────
import { useEffect, useState } from "react";
import {
  Plus, Edit3, Archive, CheckCircle, AlertTriangle,
  Building2, Wallet, RefreshCw, X,
} from "lucide-react";
...
const EMPTY_FORM = {
  ...
  account_type: "mas" as AccountType,
  officer_name: "", opening_balance: "",
...
export default function AccountsPanel({ supabase, canCRUD }: Props) {
  ...
  const openEdit = (a: Account) => {
    setEditing(a);
    setForm({
      name: a.name, bank_name: a.bank_name,
    ...
    <div>
          ...
          {(["mas","aof","cash"] as AccountType[]).map(type => (
            <div key={type}>
              ...
              <div style={{
                ...
                borderBottom:`2px solid ${TYPE_COLORS[type]}22`,
              }}>
                <div style={{
              ...
              {grouped[type].length === 0 ? (
                <div style={{
                  ...
                  background:"rgba(26,92,42,0.03)", borderRadius:10,
                  border:"1.5px dashed rgba(26,92,42,0.12)",
                  color:"var(--muted)", fontSize:"0.82rem",
                }}>
                  No {TYPE_LABELS[type]} accounts yet.
                  {canCRUD && " Click \"Add Account\" to create one."}
                </div>
              ) : (
                    ...
                    <div key={a.id} style={{
                      ...
                      <div style={{ padding:"1rem 1.2rem", borderBottom:"1px solid rgba(26,92,42,0.06)", background:"var(--warm)" }}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                          <div>
                            <p style={{ fontSize:"0.82rem", fontWeight:700, color:"var(--green-dk)" }}>{a.name}</p>
                            <p style={{ fontSize:"0.72rem", color:"var(--muted)", marginTop:2 }}>
                              {a.bank_name}
                              {a.account_number && ` · ${a.account_number}`}
                              {a.officer_name && ` · ${a.officer_name}`}
                            </p>
                          </div>
                          {!a.is_active && (
                            <span style={{ fontSize:"0.6rem", fontWeight:700, background:"rgba(192,57,43,0.1)", color:"#C0392B", padding:"2px 7px", borderRadius:4 }}>
                              ARCHIVED
                            </span>
                          )}
                        </div>
                      ...
                      {/* Balance */}
                      <div style={{ padding:"1.2rem", display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"0.5rem" }}>
                        <div style={{ textAlign:"center" }}>
                          <p style={{ fontSize:"0.6rem", fontWeight:700, letterSpacing:"0.08em", textTransform:"uppercase", color:"var(--muted)", marginBottom:4 }}>Opening</p>
                          <p style={{ fontSize:"0.82rem", fontWeight:700, color:"var(--text)" }}>{peso(a.opening_balance)}</p>
                        </div>
                        <div style={{ textAlign:"center", borderLeft:"1px solid rgba(26,92,42,0.08)", borderRight:"1px solid rgba(26,92,42,0.08)" }}>
                          <p style={{ fontSize:"0.6rem", fontWeight:700, letterSpacing:"0.08em", textTransform:"uppercase", color:"var(--muted)", marginBottom:4 }}>Net Movement</p>
                          <p style={{ fontSize:"0.82rem", fontWeight:700, color: a.total_income >= a.total_expenses ? "#2E8B44" : "#C0392B" }}>
                            {a.total_income >= a.total_expenses ? "+" : "−"}{peso(Math.abs(a.total_income - a.total_expenses))}
                          </p>
                        </div>
                        <div style={{ textAlign:"center" }}>
                          <p style={{ fontSize:"0.6rem", fontWeight:700, letterSpacing:"0.08em", textTransform:"uppercase", color:"var(--muted)", marginBottom:4 }}>Balance</p>
                          <p style={{ fontSize:"0.9rem", fontWeight:800, color:"var(--green-dk)" }}>{peso(a.current_balance)}</p>
                        </div>
                      </div>

                      {/* Opening reference */}
                      {a.opening_reference && (
                        <div style={{ padding:"0 1.2rem 0.8rem", fontSize:"0.68rem", color:"var(--muted)" }}>
                          📋 {a.opening_reference}
                        </div>
                      )}
                      ...
                      {/* Actions */}
                      {canCRUD && (
                        <div style={{ padding:"0.8rem 1.2rem", borderTop:"1px solid rgba(26,92,42,0.06)", display:"flex", gap:"0.5rem" }}>
                          <button onClick={() => openEdit(a)} style={{
                            display:"flex", alignItems:"center", gap:4,
                            background:"none", border:"1px solid rgba(26,92,42,0.2)",
                            color:"var(--green-dk)", padding:"0.35rem 0.8rem",
                            borderRadius:6, fontSize:"0.72rem", cursor:"pointer",
                            fontFamily:"'DM Sans',sans-serif",
                          }}>
                            <Edit3 size={11} /> Edit
                          </button>
                          <button onClick={() => handleArchive(a)} style={{
                            display:"flex", alignItems:"center", gap:4,
                            background:"none", border:`1px solid ${a.is_active ? "rgba(192,57,43,0.3)" : "rgba(46,139,68,0.3)"}`,
                            color: a.is_active ? "#C0392B" : "#2E8B44",
      ...
      {showForm && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:"2rem" }}>
          <div style={{ background:"white", borderRadius:14, maxWidth:540, width:"100%", maxHeight:"90vh", overflowY:"auto" }}>
            ...
            <div style={{ padding:"1.6rem", display:"flex", flexDirection:"column", gap:"1.2rem" }}>
                  ...
                  {(["mas","aof","cash"] as AccountType[]).map(t => (
                    <button key={t} onClick={() => setForm(p => ({ ...p, account_type: t }))}
                      style={{
                        ...
                        border:`2px solid ${form.account_type === t ? TYPE_COLORS[t] : "rgba(26,92,42,0.12)"}`,
                        background: form.account_type === t ? `${TYPE_COLORS[t]}11` : "white",
                        color: form.account_type === t ? TYPE_COLORS[t] : "var(--muted)",
                        fontSize:"0.75rem", fontWeight:700,
                        fontFamily:"'DM Sans',sans-serif",
                      }}>
                      {TYPE_LABELS[t]}
              ...
              <div>
                ...
                <input style={inputStyle} value={form.name}
                  placeholder={form.account_type === "cash" ? "Petty Cash Fund" : "e.g. BDO MAS Savings Account"}
              ...
              <div>
                <label style={labelStyle}>
                  {form.account_type === "cash" ? "Location / Description *" : "Bank Name *"}
                </label>
                <input style={inputStyle} value={form.bank_name}
                  placeholder={form.account_type === "cash" ? "e.g. Office Safe" : "e.g. BDO, Metrobank, Landbank"}
              ...
              {/* Officer name (cash only) */}
              {form.account_type === "cash" && (
                <div>
                  <label style={labelStyle}>Custodian / Officer Name</label>
                  <input style={inputStyle} value={form.officer_name}
                    placeholder="e.g. Anrora Puertos"
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1rem" }}>
                <div>
                  ...
                  <input style={inputStyle} type="number" min={0} value={form.opening_balance}
                    placeholder="0.00"
                    onChange={e => setForm(p => ({ ...p, opening_balance: e.target.value }))} />
                </div>
                <div>
                  <label style={labelStyle}>As of Date *</label>
                  <input style={inputStyle} type="date" value={form.opening_balance_date}
                    onChange={e => setForm(p => ({ ...p, opening_balance_date: e.target.value }))} />
                </div>
              </div>
              ...
              <div>
                ...
                <input style={inputStyle} value={form.opening_reference}
                  placeholder="e.g. General Assembly Resolution No. 2024-001"
                  onChange={e => setForm(p => ({ ...p, opening_reference: e.target.value }))} />
                <p style={{ fontSize:"0.68rem", color:"var(--muted)", marginTop:4 }}>
                  BOD resolution or document that authorized this opening balance.
                </p>
              </div>
