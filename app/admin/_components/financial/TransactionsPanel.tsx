...
interface Transaction {
  ...
  reference_no: string; receipt_url: string | null;
  receipt_uploaded_at: string | null;
  is_from_payments: boolean; created_at: string;
...
export default function TransactionsPanel({ supabase, canCRUD }: Props) {
  ...
  const cats = selectedAccount ? (CATEGORIES[selectedAccount.account_type]?.[form.direction] || []) : [];
  ...
  const openEdit = (t: Transaction) => {
    setEditing(t);
    setForm({ account_id:t.account_id, direction:t.direction, category:t.category, description:t.description||"", amount:String(t.amount), transaction_date:t.transaction_date, reference_no:t.reference_no||"" });
    setReceiptFile(t.receipt_url); setShowForm(true);
  };
  ...
  const handleReceiptUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (isImage) {
        ...
        uploadFile = await imageCompression(file,{ maxSizeMB:0.5, maxWidthOrHeight:1600, fileType:"image/webp", useWebWorker:true });
      }
  ...
  const handleSave = async () => {
    if (!form.account_id || !form.category || !form.amount) { alert("Account, category, and amount are required."); return; }
    setSaving(true); setStatus("idle");
    const payload = {
      account_id:form.account_id, direction:form.direction, category:form.category,
      description:form.description||null, amount:parseFloat(form.amount),
      transaction_date:form.transaction_date, reference_no:form.reference_no||null,
      receipt_url:receiptFile||null, receipt_uploaded_at:receiptFile?new Date().toISOString():null,
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1.2rem" }}>
        ...
        {canCRUD && (
          <button onClick={openNew} style={{ display:"flex",alignItems:"center",gap:6,background:"var(--gold)",color:"var(--green-dk)",border:"none",padding:isMobile?"0.6rem 0.9rem":"0.7rem 1.4rem",borderRadius:8,fontSize:isMobile?"0.75rem":"0.82rem",fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap" }}>
            <Plus size={14}/> {isMobile?"Add":"Add Transaction"}
          </button>
        )}
      </div>
        ...
        {[
          ...
          <div key={label} style={{ background:"white",borderRadius:10,border:"1px solid rgba(26,92,42,0.08)",padding:isMobile?"0.75rem":"1rem 1.2rem" }}>
            <div style={{ display:"flex",alignItems:"center",gap:5,marginBottom:4 }}>
              ...
              <p style={{ fontSize:"0.6rem",fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",color:"var(--muted)" }}>{label}</p>
            </div>
            <p style={{ fontSize:isMobile?"0.85rem":"1.05rem",fontWeight:800,color }}>{peso(value)}</p>
          </div>
        ))}
      </div>
      ...
      {loading ? (
            ...
            <div key={t.id} style={{ background:"white",borderRadius:10,border:"1px solid rgba(26,92,42,0.08)",borderLeft:`4px solid ${t.direction==="income"?"#2E8B44":"#C0392B"}`,padding:"0.9rem 1rem" }}>
              ...
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                ...
                {canCRUD && !t.is_from_payments && (
                  <div style={{ display:"flex",gap:6 }}>
                    <button onClick={()=>openEdit(t)} style={{ background:"none",border:"1px solid rgba(26,92,42,0.2)",color:"var(--green-dk)",padding:"0.3rem 0.6rem",borderRadius:6,fontSize:"0.7rem",cursor:"pointer" }}>
                      <Edit3 size={11}/>
                    </button>
                    <button onClick={()=>handleDelete(t)} style={{ background:"none",border:"1px solid rgba(192,57,43,0.3)",color:"#C0392B",padding:"0.3rem 0.6rem",borderRadius:6,fontSize:"0.7rem",cursor:"pointer" }}>
                      <Trash2 size={11}/>
                    </button>
                  </div>
                )}
        ...
        /* ── Desktop: table ── */
        <div style={{ background:"white",borderRadius:12,border:"1px solid rgba(26,92,42,0.08)",overflow:"auto" }}>
                  ...
                  <td style={{ padding:"0.75rem 1rem" }}>
                    {canCRUD && !t.is_from_payments && (
                      <div style={{ display:"flex",gap:"0.4rem" }}>
                        ...
                        <button onClick={()=>handleDelete(t)} style={{ background:"none",border:"1px solid rgba(192,57,43,0.3)",color:"#C0392B",padding:"0.25rem 0.5rem",borderRadius:4,fontSize:"0.68rem",cursor:"pointer" }}><Trash2 size={10}/></button>
                      </div>
                    )}
                  </td>
                </tr>
        ...
        <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:isMobile?"flex-end":"center",justifyContent:"center",padding:isMobile?0:"2rem" }}>
          <div style={{ background:"white",borderRadius:isMobile?"16px 16px 0 0":14,maxWidth:560,width:"100%",maxHeight:isMobile?"92vh":"92vh",overflowY:"auto" }}>
            <div style={{ padding:"1.2rem 1.4rem",borderBottom:"1px solid rgba(26,92,42,0.08)",display:"flex",justifyContent:"space-between",alignItems:"center",background:"var(--green-dk)",borderRadius:isMobile?"16px 16px 0 0":"14px 14px 0 0",position:"sticky",top:0,zIndex:1 }}>
              ...
              <button onClick={()=>setShowForm(false)} style={{ background:"none",border:"none",color:"rgba(255,255,255,0.6)",cursor:"pointer" }}><X size={18}/></button>
            </div>
            ...
            <div style={{ padding:"1.2rem",display:"flex",flexDirection:"column",gap:"1rem" }}>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.5rem" }}>
                  {(["income","expense"] as const).map(d=>(
                    <button key={d} onClick={()=>setForm(p=>({...p,direction:d,category:""}))}
                      style={{ padding:"0.75rem",borderRadius:8,cursor:"pointer",border:`2px solid ${form.direction===d?(d==="income"?"#2E8B44":"#C0392B"):"rgba(26,92,42,0.12)"}`,background:form.direction===d?(d==="income"?"rgba(46,139,68,0.08)":"rgba(192,57,43,0.08)"):"white",color:form.direction===d?(d==="income"?"#2E8B44":"#C0392B"):"var(--muted)",fontSize:"0.85rem",fontWeight:700,fontFamily:"'DM Sans',sans-serif",display:"flex",alignItems:"center",justifyContent:"center",gap:6 }}>
              ...
              <div>
                ...
                <select style={inputStyle} value={form.account_id} onChange={e=>setForm(p=>({...p,account_id:e.target.value,category:""}))}>
              ...
              <div>
                ...
                <select style={inputStyle} value={form.category} onChange={e=>setForm(p=>({...p,category:e.target.value}))} disabled={!form.account_id}>
              ...
              <div style={{ display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:"0.85rem" }}>
                <div>
                  ...
                  <input style={inputStyle} value={form.description} placeholder="e.g. Juan dela Cruz" onChange={e=>setForm(p=>({...p,description:e.target.value}))}/>
                </div>
                <div>
                  <label style={labelStyle}>Reference No.</label>
                  <input style={inputStyle} value={form.reference_no} placeholder="e.g. OR-001" onChange={e=>setForm(p=>({...p,reference_no:e.target.value}))}/>
                </div>
              </div>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.85rem" }}>
                <div>
                  ...
                  <input style={inputStyle} type="number" min={0} value={form.amount} placeholder="0.00" onChange={e=>setForm(p=>({...p,amount:e.target.value}))}/>
                </div>
                <div>
                  <label style={labelStyle}>Date *</label>
                  <input style={inputStyle} type="date" value={form.transaction_date} onChange={e=>setForm(p=>({...p,transaction_date:e.target.value}))}/>
                </div>
              </div>
              ...
              {/* Buttons */}
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.8rem",paddingTop:"0.25rem" }}>
