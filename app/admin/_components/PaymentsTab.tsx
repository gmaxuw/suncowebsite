...
// Better presentation, cleaner table,
// improved modal design, OR number required.
// ─────────────────────────────────────────────
import { useEffect, useState } from "react";
import { CreditCard, PlusCircle, Search, X, Check, TrendingUp, Calendar, Hash } from "lucide-react";
...
const PAYMENT_TYPES: { type: PaymentType; label: string; desc: string; amount: number; color: string }[] = [
  { type: "lifetime", label: "Lifetime Membership", desc: "One-time only. Cannot be paid again once recorded.", amount: 200,  color: "#6B3FA0" },
  { type: "aof",      label: "Annual Operating Fund (AOF)", desc: "Paid annually. Covers organizational operations.", amount: 100,  color: "#2B5FA8" },
  { type: "mas",      label: "Mortuary Assistance (MAS)", desc: "Annual mutual aid contribution for member families.", amount: 740,  color: "#2E8B44" },
];
...
export default function PaymentsTab({ canCRUD, supabase, currentUser, currentRole, currentMemberName }: Props) {
  ...
  const selectMember = async (member: any) => {
    ...
    const { data: existing } = await supabase
      .from("payments").select("type, year, amount, date_paid, receipt_number")
      .eq("member_id", member.id).order("year", { ascending: false });
  ...
  const totalAmount = selectedTypes.reduce((sum, type) => {
    return sum + (PAYMENT_TYPES.find(p => p.type === type)?.amount || 0);
  }, 0);
  ...
  const handleSave = async () => {
    ...
    const inserts = selectedTypes.map(type => ({
      ...
      date_paid:      datePaid,
      receipt_number: selectedTypes.length > 1 ? `${orNumber.trim()}-${type.toUpperCase()}` : orNumber.trim(),
    ...
    await logActivity("PAYMENT_RECORDED", {
      ...
      year, types: selectedTypes, total_amount: totalAmount,
      or_number: orNumber.trim(), date_paid: datePaid,
    });

    setSelectedMember(null); setMemberQuery(""); setSelectedTypes([]);
    setOrNumber(""); setDatePaid(new Date().toISOString().split("T")[0]);
    setYear(new Date().getFullYear()); setMemberExistingPays([]);
    setShowForm(false);
    await loadData();
    setSaving(false);
  };
  ...
  const thisYearTotal  = payments.filter(p => p.year === thisYear).reduce((s, p) => s + Number(p.amount), 0);
  ...
  return (
    <div style={{ fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ marginBottom: "1.2rem", display: "flex", alignItems: "center", gap: 8, background: "white", border: "1.5px solid rgba(26,92,42,0.15)", borderRadius: 8, padding: "0 1rem", maxWidth: 420, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
        ...
        {searchHistory && <button onClick={() => setSearchHistory("")} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, display: "flex" }}><X size={14} color="var(--muted)" /></button>}
      </div>
      ...
      <div style={{ background: "white", borderRadius: 12, border: "1px solid rgba(26,92,42,0.08)", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
        <div style={{ padding: "1.1rem 1.5rem", borderBottom: "1px solid rgba(26,92,42,0.07)", background: "#F9F8F5", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <p style={{ fontSize: "0.72rem", color: "var(--muted)", marginTop: 2 }}>{filteredPayments.length} entries{searchHistory ? ` matching "${searchHistory}"` : ""}</p>
          </div>
        </div>
        {loading ? (
            ...
            <div style={{ padding: "1.6rem 1.8rem" }}>
              ...
              <div style={{ marginBottom: "1.5rem" }}>
                ...
                <select value={year} onChange={e => setYear(Number(e.target.value))} style={inputStyle}>
                  {Array.from({ length: 31 }, (_, i) => new Date().getFullYear() - i).map(y => <option key={y} value={y}>{y}</option>)}
                </select>
              </div>
              ...
              <div style={{ marginBottom: "1.5rem" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
                  {PAYMENT_TYPES.map(({ type, label, desc, amount, color }) => {
                    ...
                    return (
                      <button key={type} onClick={() => toggleType(type)} disabled={!selectedMember || alreadyPaid}
                        style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.9rem 1.1rem", borderRadius: 10, border: `1.5px solid ${isSelected ? color : alreadyPaid ? "rgba(0,0,0,0.07)" : "rgba(26,92,42,0.12)"}`, background: isSelected ? `${color}10` : alreadyPaid ? "rgba(0,0,0,0.02)" : "white", cursor: !selectedMember || alreadyPaid ? "not-allowed" : "pointer", opacity: !selectedMember ? 0.5 : alreadyPaid ? 0.6 : 1, textAlign: "left", width: "100%", fontFamily: "'DM Sans',sans-serif" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{ width: 22, height: 22, borderRadius: 6, border: `2px solid ${isSelected ? color : "rgba(26,92,42,0.2)"}`, background: isSelected ? color : "white", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                            {isSelected && <Check size={12} color="white" />}
                          </div>
                          <div>
                            <p style={{ fontSize: "0.85rem", fontWeight: 600, color: alreadyPaid ? "var(--muted)" : "var(--green-dk)" }}>{label}</p>
                        ...
                        <div style={{ textAlign: "right", flexShrink: 0 }}>
                          <p style={{ fontFamily: "'Playfair Display',serif", fontSize: "1.15rem", fontWeight: 700, color: isSelected ? color : alreadyPaid ? "var(--muted)" : "var(--green-dk)" }}>₱{amount.toLocaleString()}</p>
              ...
              {selectedTypes.length > 0 && (
                <div style={{ background: "linear-gradient(135deg,#0D3320,#1A5C2A)", borderRadius: 12, padding: "1.4rem", marginBottom: "1.5rem" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.8rem", marginBottom: "0.8rem" }}>
                    ...
                    {selectedTypes.map(type => {
                      ...
                      return (
                        <div key={type} style={{ display: "flex", justifyContent: "space-between", fontSize: "0.85rem", marginBottom: "0.4rem" }}>
                          <span style={{ color: "rgba(255,255,255,0.65)" }}>{pt.label}</span>
                          <span style={{ color: pt.color === "#C9A84C" ? "#C9A84C" : "rgba(255,255,255,0.85)", fontWeight: 600 }}>₱{pt.amount.toLocaleString()}</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.8rem" }}>
                ...
                <button onClick={handleSave} disabled={saving || !selectedMember || selectedTypes.length === 0 || !orNumber.trim()}
                  style={{ padding: "0.85rem", background: saving || !selectedMember || selectedTypes.length === 0 || !orNumber.trim() ? "#CCC" : "var(--gold)", border: "none", color: "var(--green-dk)", borderRadius: 8, fontSize: "0.85rem", fontWeight: 700, cursor: saving || !selectedMember || selectedTypes.length === 0 || !orNumber.trim() ? "not-allowed" : "pointer", fontFamily: "'DM Sans',sans-serif" }}>
