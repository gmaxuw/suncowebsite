...
function fileTypeLabel(type: string) {
  return { pdf:"PDF", image:"Image", word:"Word", excel:"Spreadsheet", other:"File" }[type] || "File";
}

// ── Download Gate Modal (identical to PostPageClient) ─────────
...
// ── Document Download Section (identical to PostPageClient) ───
...
function RenderBlock({ block }: { block: ContentBlock }) {
  ...
  if (block.type === "heading") return (
    <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: "1.35rem", fontWeight: 700, color: "#0D3320", margin: "2rem 0 0.8rem", paddingBottom: "0.4rem", borderBottom: "2px solid #C9A84C", textAlign: align }}
      dangerouslySetInnerHTML={{ __html: block.content }} />
  );
  if (block.type === "text") return (
    <p style={{ fontFamily: "'Source Serif 4',Georgia,serif", fontSize: "1.08rem", lineHeight: 1.9, color: "#2A2A2A", marginBottom: "1.4rem", fontWeight: 300, textAlign: align }}
      dangerouslySetInnerHTML={{ __html: block.content.replace(/\n/g, "<br/>") }} />
  );
  if (block.type === "image") return (
    <figure style={{ margin: "2rem 0", textAlign: align }}>
      {block.image_url && <img src={block.image_url} alt={block.content || ""} loading="lazy" style={{ maxWidth: "100%", borderRadius: 12, boxShadow: "0 6px 24px rgba(0,0,0,0.14)", display: "block", margin: align === "center" ? "0 auto" : undefined }} />}
      {block.content && <figcaption style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "0.8rem", color: "#888", marginTop: "0.65rem", fontStyle: "italic" }}>{block.content}</figcaption>}
    </figure>
  );
  if (block.type === "two-col") return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: "2rem", margin: "1.5rem 0" }}>
      <div style={{ fontFamily: "'Source Serif 4',Georgia,serif", fontSize: "1.05rem", color: "#2A2A2A", lineHeight: 1.9, fontWeight: 300 }} dangerouslySetInnerHTML={{ __html: (block.content || "").replace(/\n/g, "<br/>") }} />
      <div style={{ fontFamily: "'Source Serif 4',Georgia,serif", fontSize: "1.05rem", color: "#2A2A2A", lineHeight: 1.9, fontWeight: 300 }} dangerouslySetInnerHTML={{ __html: (block.content2 || "").replace(/\n/g, "<br/>") }} />
    </div>
  );
  return null;
}
    ...
    <>
      ...
      <div style={{ background: "#F7F5F0", minHeight: "100vh", fontFamily: "'DM Sans',sans-serif" }}>
        ...
        <div className="p3col">
          ...
          {/* Left column — ads + share + downloads */}
          ...
          {/* Right column — clock + recent articles + right ads + SUNCO card */}
